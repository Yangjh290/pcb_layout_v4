...
def plot_polygons(original: Polygon, scaled: Polygon):
    ...
    # 绘制缩放后的多边形
    plot_polygon(scaled, ax=ax, add_points=True, color='green', alpha=0.5, label='after scaled')
...
def amplify_board_for_line(original_polygon, scale_factor):
    ...
    try:
        ...
        # 打印结果
        general_logger.info("原始多边形顶点", original_polygon)
        general_logger.info("放大后的多边形顶点", scaled_polygon)

        # 绘制
        plot_polygons(original_polygon_obj, scaled_polygon_obj)
    except (ValueError, TypeError) as e:
        general_logger.error(f"输入数据有误: {e}")
